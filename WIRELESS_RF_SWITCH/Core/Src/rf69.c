...
uint8_t spiRead(uint8_t reg)
{
	...
	HAL_SPI_Transmit(&hspi1, &TxDt,sizeof(TxDt), 1000);                   // Send the address with the write mask off
	HAL_SPI_Receive(&hspi1, &val, sizeof(val), 1000);                     // The written value is ignored, reg value is read
...
uint8_t spiWrite(uint8_t reg, uint8_t val)
{
	...
	TxDt = (reg | RH_RF69_SPI_WRITE_MASK);
	HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_TransmitReceive(&hspi1, &TxDt, &status, sizeof(TxDt), 1000); // Send the address with the write mask on
	HAL_SPI_Transmit(&hspi1, &val, sizeof(val), 1000);                   // New value follows
	HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_SET);
	return status;
}
...
uint8_t spiBurstRead(uint8_t reg, uint8_t* dest, uint8_t len)
{
	...
	TxDt = (reg & ~RH_RF69_SPI_WRITE_MASK);
	HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_TransmitReceive(&hspi1, &TxDt, &status, sizeof(TxDt), 1000); // Send the start address with the write mask off
	while (len--)
	{
		HAL_SPI_TransmitReceive(&hspi1, &dummyData, &rxDt, sizeof(rxDt), 1000);
		*dest++ = rxDt;
	}
	HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_SET);
	return status;
...
uint8_t spiBurstWrite(uint8_t reg, const uint8_t* src, uint8_t len)
{
	...
	TxDt = (reg | RH_RF69_SPI_WRITE_MASK);
	HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_TransmitReceive(&hspi1, &TxDt, &status, sizeof(TxDt), 1000);  // Send the start address with the write mask on
	while (len--)
	{
		TxDt1 = *src++;
		HAL_SPI_Transmit(&hspi1, &TxDt1, sizeof(TxDt1), 1000);            // New value follows
	}
	HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_SET);
	return status;
}
...
void readFifo()
{
	...
	HAL_SPI_Transmit(&hspi1, &Data, sizeof(Data), 100) ;			// Send the start address with the write mask off
	HAL_SPI_Receive(&hspi1, &payloadlen, sizeof(payloadlen), 100);	// First byte is payload len (counting the headers)
	//safe_printf("PayloadLen:%d\n",payloadlen);
	if (payloadlen <= RH_RF69_MAX_ENCRYPTABLE_PAYLOAD_LEN && payloadlen >= RH_RF69_HEADER_LEN)
	{
		HAL_SPI_Receive(&hspi1, &_rxHeaderTo, sizeof(_rxHeaderTo), 100);
		//safe_printf("_rxHeaderTo:%d\n",_rxHeaderTo);
		// Check addressing
		if (_promiscuous ||	_rxHeaderTo == _thisAddress ||_rxHeaderTo == RH_BROADCAST_ADDRESS)
		{
			// Get the rest of the headers
			HAL_SPI_Receive(&hspi1, &_rxHeaderFrom, sizeof(_rxHeaderFrom), 1000);
			//safe_printf("_rxHeaderFrom:%d\n",_rxHeaderFrom);
			HAL_SPI_Receive(&hspi1, &_rxHeaderId, sizeof(_rxHeaderId), 1000);
			//safe_printf("_rxHeaderId:%d\n",_rxHeaderId);
			HAL_SPI_Receive(&hspi1, &_rxHeaderFlags, sizeof(_rxHeaderFlags), 1000);
			//safe_printf("_rxHeaderFlags:%d\n",_rxHeaderFlags);

			// And now the real payload
			for (_bufLen = 0; _bufLen < (payloadlen - RH_RF69_HEADER_LEN); _bufLen++)
			{
				HAL_SPI_Receive(&hspi1, &_buf[_bufLen], 1, 1000);
			}
				_rxGood++;
				_rxBufValid = true;
		}
...
bool send1(const uint8_t* data, uint8_t len)
{
	...
	printf( "_txHeaderTo    =%d\n", _txHeaderTo);
	printf( "_txHeaderFrom  =%d\n", _txHeaderFrom);
	printf( "_txHeaderId    =%d\n", _txHeaderId);
	printf( "_txHeaderFlags =%d\n", _txHeaderFlags);

	HAL_GPIO_WritePin(RF_CS_GPIO_Port, RF_CS_Pin, GPIO_PIN_RESET);
	TxDt = (RH_RF69_REG_00_FIFO | RH_RF69_SPI_WRITE_MASK);
	HAL_SPI_Transmit(&hspi1, &TxDt, sizeof(TxDt), 1000);
	TxDt = (len + RH_RF69_HEADER_LEN);                 // Include length of headers
	HAL_SPI_Transmit(&hspi1, &TxDt, sizeof(TxDt), 1000);

	HAL_SPI_Transmit(&hspi1, &_txHeaderTo, 	 sizeof(_txHeaderTo),   1000);
	HAL_SPI_Transmit(&hspi1, &_txHeaderFrom, sizeof(_txHeaderFrom), 1000);
	HAL_SPI_Transmit(&hspi1, &_txHeaderId, 	 sizeof(_txHeaderId),   1000);
	HAL_SPI_Transmit(&hspi1, &_txHeaderFlags,sizeof(_txHeaderFlags),1000);
